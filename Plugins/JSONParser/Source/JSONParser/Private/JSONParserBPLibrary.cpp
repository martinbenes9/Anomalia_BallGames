...
FString UJSONParserBPLibrary::JsonObjectToString(bool& successful, struct FSJsonObject mainObject, bool addPadding, bool compactString) {
	...
	if (compactString) {
		TSharedRef<TJsonWriter<TCHAR, TCondensedJsonPrintPolicy<TCHAR>>> json_writer = TJsonWriterFactory<TCHAR, TCondensedJsonPrintPolicy<TCHAR>>::Create(&data);
